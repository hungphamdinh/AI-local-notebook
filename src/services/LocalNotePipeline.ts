...
export const LocalNotePipeline={
  ...
  async generateSummary(documentId:string,type:GenerationType,onToken?:(token:string)=>void,onProgress:(value:number)=>void=()=>{}):Promise<string>{const chunks=await DocumentRepository.chunks(documentId);if(!chunks.length)throw new LocalNoteError('context_too_long','Document processing is incomplete.','Wait for extraction and retry.',true);
    const chunkSummaries:string[]=[];for(let i=0;i<chunks.length;i+=1){const chunk=chunks[i]!;chunkSummaries.push(await InferenceService.generate(summaryPrompt('section_summary',`[source:${chunk.id}] ${chunk.text}`),undefined,180));onProgress(.05+.55*(i+1)/chunks.length);}
    let level=chunkSummaries;while(level.join('\n\n').length>12000){const next:string[]=[];for(let i=0;i<level.length;i+=6)next.push(await InferenceService.generate(summaryPrompt('section_summary',level.slice(i,i+6).join('\n\n')),undefined,240));level=next;onProgress(.72);}
    onProgress(.8);const result=await InferenceService.generate(summaryPrompt(type,level.join('\n\n')),onToken,600);await ContentRepository.save(documentId,type,result,await InferenceService.modelId(),chunks.map(c=>c.id));onProgress(.98);return result;},
  ...
  async ask(documentId:string,question:string,onToken?:(token:string)=>void,onProgress:(value:number)=>void=()=>{}):Promise<{answer:string;citations:Citation[]}>{if(!question.trim())throw new LocalNoteError('context_too_long','Question is empty.','Enter a question about this document.');
    const chunks=await DocumentRepository.chunks(documentId);if(chunks.some(c=>!c.embedding))await this.ensureEmbeddings(documentId,value=>onProgress(.05+value*.45));onProgress(.55);const query=await InferenceService.embedQuery(question);const sources=await retrieve(documentId,question,query,5);onProgress(.72);
    ...
    const parsed=validateAnswer(raw,new Set(sources.map(s=>s.chunkId)));onProgress(.98);return{answer:parsed.answer,citations:sources.filter(s=>parsed.sourceIds.includes(s.chunkId))};},
};
